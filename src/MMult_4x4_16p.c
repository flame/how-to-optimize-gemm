...
void AddDot4x4( int k, double *a, int lda,  double *b, int ldb, double *c, int ldc )
{
  ...
  int i,j,p;
  double cc[4][4] __attribute__((aligned(64)));
  for (i=0; i<4; i++) {
    for (j=0; j<4; j++) cc[i][j] = c[i*ldc+j];
  }
  for (p=0; p<k; p++) {
    double ca[4] __attribute__((aligned(64)));
    for (j=0; j<4; j++) ca[j] = a[p*lda+j];
    for (i=0; i<4; i++) {
      for (j=0; j<4; j++) cc[i][j] += ca[j] * b[p*4+i];
    }
  }
  for (i=0; i<4; i++) {
    for (j=0; j<4; j++) c[i*ldc+j] = cc[i][j];
  }
}
